// package gethwrap contains wraaper of Geth operations
package gethwrap

import (
	"context"
	"fmt"
	"goethdevkit/internal/shared"
	"log"
...
// InfuraQueryTxnByHash query transaction via infura
//
// Arguments:
//
// * infuralNetURL url to node via infura end point
// * infuraPID     infura project ID
// * limits        the maximum number of calls to infura end, if less than 1 it defaults to 1
//
func InfuraQueryTxnByHash(infuralNetURL string, infuraPID string, txnHash string, limit int) (*shared.TransactionInfo, error) {
